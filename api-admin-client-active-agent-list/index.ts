...
import { getAdminClientActiveAgentList } from '../src/interface/response/agentclient.response.interface';
import { AdminClientActiveAgentController } from '../src/controller/agentClient/get.admin.client.active.agent.controller';
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
    ...
    /**
     * Role - Admin
     * Get client active agent list
     */
    const agents: getAdminClientActiveAgentList[] =
      await new AdminClientActiveAgentController(context).run();
    console.log('Success', agents);
    context.res = await baseResponse({ agents }, 200, {
      'Content-Type': 'application/json'
    });
  } catch (ex) {
    console.log('Get client active agent list index file catch block', ex);
