...
import { UpdateAdminAgentThumbController } from '../src/controller/agent/update.admin.agent.thumb.controller';
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
     ...
     * Updates agent thumb
     */
    const result: AgentThumbResponse =
      await new UpdateAdminAgentThumbController(context).run();
    console.log('Success', result);
    context.res = await baseResponse(result, 200, {
      'Content-Type': 'application/json'
    });
  } catch (ex) {
    console.log('Updates agent thumb index file catch block', ex);
