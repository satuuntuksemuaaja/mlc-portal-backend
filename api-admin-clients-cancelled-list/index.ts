...
import { AdminCancelledClientsListController } from '../src/controller/client/get.admin.cancelled.clients.list.controller';
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
    ...
    /**
     * Role - Admin
     * Cancelled client list
     */
    const clients: ClientResponse =
      await new AdminCancelledClientsListController(context).run();
    console.log('Success', clients);
    context.res = await baseResponse({ clients }, 200, {
      'Content-Type': 'application/json'
    });
  } catch (ex) {
    console.log('Cancelled client list index file - catch block', ex);
