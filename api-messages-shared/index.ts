...
import { GetSharedMessagesController } from '../src/controller/message/get.sharedMessages.controller';
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
    ...
    const resp = await new GetSharedMessagesController(context).run();
    ...
    if (ex?.statusCode) {
      ...
      context.res = errorResponse({ message: 'Internal Server Error' }, 500);
    }
  } finally {
    /**
