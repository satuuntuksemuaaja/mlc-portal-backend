...
import { UpdateClientController } from '../src/controller/client/update.client.controller';
  ...
  try {
    ...
    if (context.req.method == 'POST') {
      const result: UserClientResponse | ErrorResponse =
        await new CreateClientController(context).run();
      ...
      const result: UserClientResponse | ErrorResponse =
        await new UpdateClientController(context).run();
      console.log('Success', result);
      context.res = await baseResponse(result, 200, {
        'Content-Type': 'application/json'
      });
