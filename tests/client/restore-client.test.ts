...
import httpTrigger from '../../api-client-restore/index';
...
describe('Test to restore agent', () => {
  let context: Context;
  ...
  it('User level agent should be able to restore client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('Valid Admin agent should be not able to update client from other organisation', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('User level agent should be not able to update client  from other organisation', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
