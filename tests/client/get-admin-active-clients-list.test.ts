...
import httpTrigger from '../../api-admin-clients-active-list/index';
...
describe('Test to active client list', () => {
  ...
  it('Valid Admin agent should be able to list active clients - Http status code: 200', async () => {
    ...
    const activeClients = context?.res?.body?.clients.filter(
      (data) =>
        (data.status === ClientStatus.ACTIVE ||
          data.status === ClientStatus.PENDING) &&
        data.orgId === 'cd0bcea6-551d-11ed-bdc3-0242ac120002'
    );
    expect(context?.res?.body?.clients.length).toEqual(activeClients.length);
  });

  it('User level agent should fail to list active clients', async () => {
    context.req = {
      headers: {
  ...
  it('Non Registered (In DB) agent should fail to list active clients', async () => {
    context.req = {
      headers: {
