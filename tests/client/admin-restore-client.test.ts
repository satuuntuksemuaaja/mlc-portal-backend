...
import httpTrigger from '../../api-admin-client-restore/index';
...
import { BffClientSubscriptionService } from '../../src/service/subscription/subscription.bff.service';

jest.mock('../../src/service/meeco.invitation.service');
jest.mock('../../src/service/invitation/invitation.bff.service');
jest.mock('../../src/service/subscription/subscription.bff.service');

describe('Test to restore client', () => {
  let context: Context;
  ...
  it('Valid Admin agent should be able to restore client - Http status code: 200', async () => {
    ...
    (
      BffClientSubscriptionService.prototype.update as jest.Mock
    ).mockReturnValue({
      ok: true
    });
    ...
    expect(context?.res?.body?.status).toBe(ClientStatus.ACTIVE);
    expect(context?.res?.body?.name).toEqual('Test Account');
  });

  it('User level agent should fail to restore client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('Archived Admin Agent Should Fail to restore client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('Archived user agent should fail to restore client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('Non Registered (In DB) agent should fail to restore client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('Valid Admin agent should be not able to restore clientfrom other organisation', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
