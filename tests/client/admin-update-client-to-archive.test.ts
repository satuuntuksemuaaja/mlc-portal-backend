...
import httpTrigger from '../../api-admin-client-archive/index';
...
describe('Test to archive client', () => {
  ...
  it('User level agent should fail to archive client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
  ...
  it('Valid Admin agent should be not able to restore client from other organisation', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
