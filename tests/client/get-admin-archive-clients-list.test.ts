...
import httpTrigger from '../../api-admin-clients-archived-list/index';
import { ClientStatus } from '../../src/model/enums/client.enum';
import { TestData } from '../data/testdata';

describe('Test to archive client list', () => {
  let context: Context;
  ...
  it('Valid Admin agent should be able to list archive clients - Http status code: 200', async () => {
    context.req = {
      headers: {
    ...
    const archivedClients = context?.res?.body?.clients.filter(
      (data) =>
        data.status === ClientStatus.ARCHIVED &&
        data.orgId === 'cd0bcea6-551d-11ed-bdc3-0242ac120002'
    );
    expect(context?.res?.body?.clients?.length).toEqual(archivedClients.length);
  });

  it('User level agent should fail to list archive clients', async () => {
    context.req = {
      headers: {
  ...
  it('Archived Admin Agent Should Fail to list archive clients', async () => {
    context.req = {
      headers: {
  ...
  it('Archived user agent should fail to list archive clients', async () => {
    context.req = {
      headers: {
  ...
  it('Non Registered (In DB) agent should fail to list archive clients', async () => {
    context.req = {
      headers: {
