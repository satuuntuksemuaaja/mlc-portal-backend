...
describe('Test to update client details', () => {
  let context: Context;
  ...
  it('Valid Admin agent should be able to update client - Http status code: 200', async () => {
    ...
    expect(context?.res?.body.name).toBe('Update client');
  });

  it('User level agent should be able to update client', async () => {
    const newClient = await Promise.resolve(await client);
    expect(newClient).toBeDefined();
    expect(newClient.status).toBe(ClientStatus.PENDING);
    expect(newClient.ref).toBe('Abc123');
