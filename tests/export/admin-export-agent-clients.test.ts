...
import httpTrigger from '../../api-admin-export-agent-clients/index';
import { TestData } from '../data/testdata';

describe('Test to export agent client', () => {
  let context: Context;
  ...
  it('Valid Admin agent should be able to list agent client - Http status code: 200', async () => {
    context.req = {
      headers: {
    ...
    expect(context?.res?.body?.report).toEqual('agentclients');
  ...
  it('Valid Admin agent should be able to list agent client with date range - Http status code: 200', async () => {
    context.req = {
      headers: {
        authorization: new TestData().getAcme().getAdminAgent()
      },
