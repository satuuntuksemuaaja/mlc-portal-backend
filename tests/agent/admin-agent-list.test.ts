...
import httpTrigger from '../../api-admin-agents-list/index';
...
describe('Test to agent list', () => {
  ...
  it('Valid Admin agent should be able to list agents - Http status code code: 200', async () => {
    context.req = {
      headers: {
    ...
    const agents = context?.res?.body?.agents.filter(
      (data) => data.orgId != 'cd0bcea6-551d-11ed-bdc3-0242ac120002'
    );
    expect(0).toEqual(agents.length);
  });
