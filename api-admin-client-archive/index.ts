...
import { AdminUpdateClientToArchiveController } from '../src/controller/client/update.admin.archived.client.controller';
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
    ...
    /**
     * Role - Admin
     * Update client to archive
     */
    const result: ClientResponse =
      await new AdminUpdateClientToArchiveController(context).run();
    console.log('Success', result);
    context.res = await baseResponse(result, 200, {
      'Content-Type': 'application/json'
    });
  } catch (ex) {
    console.log('Update client to archive index file catch block', ex);
