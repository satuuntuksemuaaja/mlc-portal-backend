...
import { UpdateClientToArchivedController } from '../src/controller/client/update.archived.client.controller';
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
     ...
     * Archive an client
     */
    const result: UserClientResponse =
      await new UpdateClientToArchivedController(context).run();
    console.log('Success', result);
    context.res = await baseResponse(result, 200, {
      'Content-Type': 'application/json'
    });
  } catch (ex) {
    console.log('Archive an client index file - catch block', ex);
